    ...
    public class ForumRepository : RepositoryBase, IForumRepository
    {
        ...
        public void DeleteForum(Forum forum)
        {
            ...
            this.InvokeTransaction(
                delegate(ISession session)
                {
                    Enumerable.ToList<Topic>(session.Query<Topic>().Where(t => t.Forum.Equals(forum))).ForEach(t => this.DeleteTopic(t, session));
            ...
            this.InvokeTransaction(
                delegate(ISession session)
                {
                    ...
                    if (this.PostWasTheFirstInTopic(session, post))
                    {
                        ...
                        Post lastPost =
                            Queryable.First<Post>(session.Query<Post>().Where(p => p.Topic.Forum.Id == post.Topic.Forum.Id).OrderByDescending(
                                    p => p.Created).ThenByDescending(p => p.Id));
                    ...
                    {
                        Post lastPost =
                            Queryable.First<Post>(session.Query<Post>().Where(p => p.Topic.Id == post.Topic.Id).OrderByDescending(
                                    p => p.Created).ThenByDescending(p => p.Id));
        ...
        {
            List<Topic> topics = Enumerable.ToList<Topic>(this.Session.Query<Topic>().Where(t => !t.ExcludedUsers.Contains(currentUser)).OrderByDescending(t => t.LastPostTime).Take(10));
        ...
        {
            List<Forum> allForums = Enumerable.ToList<Forum>(this.Session.Query<Forum>());
        ...
        public Post GetFirstUnreadPostForTopic(int id, MediaCommUser user)
        {
            ...
            DateTime date =
                (Queryable.SingleOrDefault<TopicRead>(this.Session.Query<TopicRead>(), tr => tr.ReadByUser.UserName == user.UserName && tr.ReadTopic.Id == id) ??
                 new TopicRead { LastVisit = DateTime.Now.AddMonths(-1) }).LastVisit;

            Post post =
                Queryable.FirstOrDefault<Post>(this.Session.Query<Post>().Where(p => p.Topic.Id == id && p.Created > date).OrderByDescending(p => p.Created));
        ...
        public int GetLastPageNumberForTopic(int topicId, int pageSize)
        {
            ...
            int totalPostCount = Queryable.Count<Post>(this.Session.Query<Post>().Where(p => p.Topic.Id == topicId));
        ...
        {
            List<Post> posts = Enumerable.ToList<Post>(this.Session.Query<Post>().Where(p => p.Topic.Id == topicId).OrderBy(p => p.Created));
        ...
        public IEnumerable<Post> GetPostsForTopic(int topicId, PagingParameters pagingParameters, MediaCommUser currentUser)
        {
            ...
            List<Post> posts =
                Enumerable.ToList<Post>(this.Session.Query<Post>().Where(p => p.Topic.Id == topicId)
                        .OrderBy(p => p.Created)
                        .Skip(postsToSkip)
            ...
            if (isLastPage)
            {
                this.InvokeTransaction(
                    ...
                    {
                        TopicRead topicRead =
                            Queryable.SingleOrDefault<TopicRead>(s.Query<TopicRead>(), r =>
...
#warning check excluded users
        ...
        public IEnumerable<Topic> GetTopicsForForum(int forumId, PagingParameters pagingParameters, MediaCommUser currentUser)
        {
            ...
            List<Topic> topics =
                Enumerable.ToList<Topic>(this.Session.Query<Topic>().Where(
        ...
        private void DeleteTopic(Topic topic, ISession session)
        {
            ...
            Enumerable.ToList<Post>(session.Query<Post>().Where(p => p.Topic.Id == topic.Id)).ForEach(session.Delete);
